...
class si_cf_moduloModel
{
   ...
   return $arr;
	...
	$this->db->query("SELECT IDMODULO,NOMBRE,IDPADRE FROM SI_CF_MODULO WHERE IDMODULO = '$IDModulo'");
	...
	if( $r_padre['IDPADRE'] != 0 )
	{
		...
		$this->db->query("SELECT IDMODULO,NOMBRE,IDPADRE FROM SI_CF_MODULO WHERE IDMODULO = '$r_padre[IDMODULO]' ");
...
//	return ("UPDATE SI_CF_MODULO SET Posicion = $pos WHERE IDModulo = $id ");
  //  }
    ...
    public function updateOper($strOper){
		...
		echo "updateOper";
		print_r($this);
		//$rs = db_query("UPDATE  SI_CF_MODULO
		//			SET Modulo = '$_POST[modulo]',
		//			Descripcion = '$_POST[Desc]',
		//			FileManual = '$_POST[File]'
		//			WHERE IDModulo = '$_POST[IDM]' ");
