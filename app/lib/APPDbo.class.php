...
class APPDbo 
{
	...
	*/
	...
	public function insertLOG($sql = '',$params = array(),$operacion = "",$IDModulo,$Tabla)
	{
		...
		//echo $sql;
		//print_r($params);
		// $transaccion = $sql.implode(" | ",(array)$params);
		$transaccion = $sql;
		
		//foreach($params AS $param => $value)
		//	$transaccion .= "$param => $value | ";
	...
	{    
		...
		$this->SQL_STRING_NOCONSTRAIN = $sql;
	...
	$SQL = $this->SQL_STRING_NOCONSTRAIN;
 
        if( !empty($Constraint) )
        {
          //  $Constraint = $this->ToPrepared($Table,$Constraint,'AND');
	  //   $SQL .= " WHERE {$Constraint[1]}";
	      $this->SQL_STRING_NOCONSTRAIN .= " WHERE {$Constraint}";
        }

	$SQL = "SELECT COUNT(*) AS NumRows FROM ( ".$this->SQL_STRING_NOCONSTRAIN.") A";
