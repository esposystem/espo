(function($){
		...
		defaultFilterOptions: {
			onInit: function(target){
				...
				function _doFilter(){
					...
					console.log(rule);
						console.log(field);
					if (rule != null && rule.param != 'undefined')
	...
	function getRuleIndex(target, field){
		...
		for(var i=0; i<rules.length; i++){
			console.log(rules[i].field + "==" + field);
	...
	function getFilterRule(target, field){
		...
		var name = getPluginName(target);
		console.log($(target)[name]('options').filterRules);
		var rules = $(target)[name]('options').filterRules;
	...
	function removeFilterRule(target, field){
		...
		if (field){
			var index = getRuleIndex(target, field);
		//	console.log('clear getRuleIndex filterRules '+ field);
		//	console.log(opts.filterRules);
		//	console.log('index' + index)
			if (index >= 0){
				// ELIMINAR ELEMENTO DE FILTRO VALIDAR POR Q COLOCABA VACIO
				opts.filterRules[index].value = "";
