...
<head>
	...
	<script type="text/javascript">
		var url;
                ...
                $(function(){
                 //   $('#dg').datagrid({ remoteFilter:true,url:'/app/pt_dm_tipo/list' });       
				...
				options:{
					panelHeight:'60px',
					loader : cloader,
					url:'/app/pt_dm_clase/list',
					valueField: 'IDCLASEPETICION',  
					textField: 'NOMBRE',
					listAll : true,
					onChange:function(value){
						if (value == ''){
							 $('#dg').datagrid('removeFilterRule', 'CLASEPETICION');
						} else {
							 $('#dg').datagrid('addFilterRule', {
								field: 'CLASEPETICION',
								op: 'join',
								value: value,
								param: 'IDCLASEPETICION'
							});
						}
		...
		function editRecord(){
			...
			if (row){
				...
				$('#fm').form('load',row);
				url = '/app/pt_dm_tipo/update/'+row.IDTIPOPETICION;
			}
		}
		function saveRecord(){
			$('#fm').form('submit',{
...
<body>
	...
	<table id="dg" title="<?php echo $opcDesc; ?>" class="easyui-datagrid" style="width:840px;height:394px"
	    ...
	    data-options="pageSize: <?php echo $config->get('GRID_NUMROWS')?>,remoteFilter:true,url:'/app/pt_dm_tipo/list'"
			toolbar="#toolbar" pagination="true"
			rownumbers="true" fitColumns="true" singleSelect="true">
		<thead>
			<tr>
    				<th field="CLASEPETICION" width="120" sortable="true">Clase de Petici&oacute;n</th>
	...
	<div id="dlg" class="easyui-dialog" style="width:560px;height:382px; padding: 0 10px 0 10px;"
			closed="true" buttons="#dlg-buttons" modal="true">
		<div class="ftitle">Datos de <?php echo $opcDesc; ?></div>
		<form id="fm" method="post" novalidate>
                        
			<div class="fitem">
				<label>Clase de Petici&oacute;n</label>
				<input id="ccb" class="easyui-combobox" name="IDCLASEPETICION" value="" required="true"
			    data-options="valueField: 'IDCLASEPETICION',  
                            textField: 'NOMBRE', 
                            width:280,
			    loader : cloader,
			    panelHeight:'150',
			...
			<div class="fitem">
				...
				<textarea name="BASE_LEGAL"  style="height:60px;" wrap="virtual"></textarea>
			</div>
			<div class="fitem">
				<label>Estado</label>
			...
			<div class="fitem">
				<label>Duraci&oacute;n Promedio Audiencia</label>
				<input name="DURACION" class="easyui-numberbox" required="true">
			</div>
		</form>
	</div>
	<div id="dlg-buttons">
