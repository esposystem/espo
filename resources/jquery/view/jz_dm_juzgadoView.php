...
<head>
	...
	<script type="text/javascript">
		var url;
                ...
                $(function(){
                 //   $('#dg').datagrid({ remoteFilter:true,url:'/app/jz_dm_juzgado/list' });       
				...
				options:{
					panelHeight:'60',
					loader : cloader,
					url:'/app/jz_dm_tipojuzgado/list',
					valueField: 'IDTIPO',  
					textField: 'NOMBRE',
					listAll : true,
					onChange:function(value){
						if (value == ''){
							 $('#dg').datagrid('removeFilterRule', 'IDTIPO');
						} else {
							 $('#dg').datagrid('addFilterRule', {
								field: 'IDTIPO',
								op: 'join',
		...
		function editRecord(){
			...
			if (row){
				...
				$('#fm').form('load',row);
				url = '/app/jz_dm_juzgado/update/'+row.IDJUZGADO;
			}
		}
		function saveRecord(){
			$('#fm').form('submit',{
...
<body>
	...
	<table id="dg" title="<?php echo $opcDesc; ?>" class="easyui-datagrid" style="width:700px;height:394px"
	    ...
	    data-options="pageSize: <?php echo $config->get('GRID_NUMROWS')?>,remoteFilter:true,url:'/app/jz_dm_juzgado/list'"
			toolbar="#toolbar" pagination="true"
			rownumbers="true" fitColumns="true" singleSelect="true">
		<thead>
			<tr>
    				<th field="IDCIUDAD" width="50" sortable="true">Ciudad</th>
				<th field="IDTIPO" width="50" sortable="true">Tipo de Juzgado</th>
	...
	<div id="dlg" class="easyui-dialog" style="width:560px;height:468px; padding: 0 10px 0 10px;"
		...
		<form id="fm" method="post" novalidate>
			...
			<div class="fitem">
				<label>Nombre</label>
				<input name="NOMBRE" class="easyui-validatebox" validType="text"  >
			</div>
			<div class="fitem">
